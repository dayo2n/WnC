$(document).ready(function(){
    ...
    // ​	 solverAdminName   해결한 관리자의 이름
            ...
            for(i=0; i<parseInt(data.totalReportCount); i++){
                ...
                if(isSolved){
                $('#reportList>tbody').prepend('<tr><td>'+reportID+'</td><td>'+targetTeacherId +' : ' + targetTeacherUsername +'</td><td>'+writerId +' : ' + writerUsername+'</td><td>'+content+'</td> <td><i class="fas fa-check" style="color: green;"></i></td><td>'+solverAdminId +' : ' +solverAdminName+'</td></tr>');
                }else{ // 아직 해결되지않은 신고이면
                $('#reportList>tbody').prepend('<tr><td>'+reportID+'</td><td>'+targetTeacherId +' : ' + targetTeacherUsername +'</td><td>'+writerId +' : ' + writerUsername+'</td><td>'+content+'</td> <td><select name="" id="blackOption"><option value="ignore" id="ignore">무시</option><option value="warn" id="warn">경고</option><option value="black" id="black">차단</option></select></td><td><input type="button" value="처리하기" id="btn-confirm"></td></tr>');
                }
            ...
            $('#btn-confirm').click(function(){
                ...
                if($('#blackOption option:selected').attr("id") === "ignore"){
                    fetch("http://219.255.114.140:8090/reports/"+reportID+"/ignore",{
                        method: "POST",
                        headers : {"Authorization" : `Bearer ${JSON.parse(localStorage.getItem("token"))}` }
                    }).then(response => {
                        console.log(response);
                    ...
                    fetch("http://219.255.114.140:8090/reports/"+reportID+"/black",{
                        method: "POST",
                        headers : {"Authorization" : `Bearer ${JSON.parse(localStorage.getItem("token"))}` }
                    }).then(response => {
                        console.log(response);
    ...
    // name   => 블랙리스트 이름
            for(i=0; i<data.length; i++){
                ...
                $('#blackList>tbody').prepend('<tr><td>'+id+'</td><td>'+username+'</td><td>'+name+'</td></tr>');
